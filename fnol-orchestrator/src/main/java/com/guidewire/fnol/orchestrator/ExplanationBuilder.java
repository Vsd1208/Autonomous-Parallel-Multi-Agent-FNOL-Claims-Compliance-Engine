...
public class ExplanationBuilder {
    ...
    public Models.ExplanationObject build(
            Models.PolicyValidationResult coverage,
            Models.DamageAssessment damage,
            Models.ReserveResult reserve,
            Models.PIIResult pii,
            Models.SubrogationResult subrogation,
            Models.PolicyHistoryContext historyContext
    ) {
        List<Models.ExplanationFactor> factors = new ArrayList<>();
        ...
        factors.add(new Models.ExplanationFactor(
                "AI Confidence",
        ...
        factors.add(new Models.ExplanationFactor(
                ...
                "PII must be redacted or absent from description"
        ));
        ...
        factors.add(new Models.ExplanationFactor(
                ...
                "Reserve must be <= $25,000 for straight-through processing"
        ));
        ...
        factors.add(new Models.ExplanationFactor(
                "Claim History",
                historyContext.claimFrequencyRisk() + " (" +
                        historyContext.claimsLast12Months() + " in 12 months)",
        ...
        String decision = allPassed ? "STRAIGHT_THROUGH" : "HUMAN_REVIEW";
        String recommendedAction = allPassed ? "APPROVE_AND_STP" : "ESCALATE_TO_HUMAN";
        String summary = allPassed
                ? "All criteria met for automated straight-through processing"
                : "One or more criteria failed; escalate to human claim handler";

        return new Models.ExplanationObject(
                decision,
                summary,
                factors,
                recommendedAction
